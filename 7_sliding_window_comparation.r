...
D1=140+sum(bit_score[D1_start:D1_end]==0.8)


if (((D1_end - D1_start)-sum(bit_score[D1_start:D1_end]==0.8)) > 140) {
  
  short_base_length_D1=D1
  sequence_length_D1=D1_end - D1_start
  base_site_D1 <- D1_start:(D1_end-D1)
  
  for (i in base_site_D1) {
    sum_min=100000
    start=i
    end=start+short_base_length_D1
    ...
    cut_end_1=start_min+short_base_length_D1
    Bits_mean_1=mean(bit_score[cut_start_1:cut_end_1])
    length_D1=cut_end_1 - cut_start_1
  }
}else{
  short_base_length_D1=140
  sequence_length_D1=350
  base_site_D1 <- begin_base_site:(sequence_length_D1-short_base_length_D1)
  sum_min=10000000
  start_min=0
  end_min=0
  for (i in base_site_D1) {
  ...
  cut_start_1=start_min
  cut_end_1=start_min+140+sum(bit_score[cut_start_1:(cut_start_1+140)]==0.8)
  Bits_mean_1=mean(bit_score[cut_start_1:cut_end_1])
  length_D1=cut_end_1 - cut_start_1
...
D2=280+sum(bit_score[D2_start:D2_end]==0.8)
if (as.integer(D1D2_mid) != as.numeric(D1D2_mid)  ){
  D1D2_mid=D1D2_mid+0.5
}

if (((D2_end-D2_start)-sum(bit_score[D2_start:D2_end]==0.8)) > 280) {
  short_base_length=D2
  base_site <- D2_start:(D2_end-short_base_length)
  sum_min=10000000
  start_min=0
  end_min=0
  for (i in base_site) {
  ...
  cut_end_2=start_min +140+ sum(bit_score[cut_start_2:(cut_start_2+140)]==0.8)
  Bits_mean_2=mean(bit_score[cut_start_2:cut_end_2])
  length_D2=cut_end_2 - cut_start_2
  cut_end_3=end_min
  cut_start_3=end_min -140- sum(bit_score[(cut_end_3-140):cut_end_3]==0.8)
  Bits_mean_3=mean(bit_score[cut_start_3:cut_end_3])
  length_D3=cut_end_3-cut_start_3
}else{
  cut_start_2=D2_start
  cut_end_2=cut_start_2 +140+ sum(bit_score[cut_start_2:(cut_start_2+140)]==0.8)
  Bits_mean_2=mean(bit_score[cut_start_2:cut_end_2])
  length_D2=cut_end_2 - cut_start_2
  cut_end_3=D2_end
  cut_start_3=cut_end_3 -140- sum(bit_score[(cut_end_3-140):cut_end_3]==0.8)
  Bits_mean_3=mean(bit_score[cut_start_3:cut_end_3])
  length_D3=cut_end_3-cut_start_3
}
