...
touch $work_dir/seq_info_in_order_level.txt
printf "Order_name","sequence_num","algned_seq_length","database_1_start","database_1_end","database_1_mean_Bits","database_1_length","database_2_start","database_2_end","database_2_mean_Bits","database_2_length","database_3_start","database_3_end","database_3_mean_Bits","database_3_length" >> $work_dir/seq_info_in_order_level.txt
...
do
  ...
  #1. seqlogo
     ...
     mv sequence_logo.pdf $work_dir/seqLogo/${Order}_seqLogo.pdf
  ...
  #3. write the selected position and information
     ...
     printf "$(cat cut_start_3.txt |awk 'NR==2{print $2}')", >> $work_dir/seq_info_in_order_level.txt
     cat cut_end_3.txt |awk 'NR==2{print $2}' > cut_end_3
     printf "$(cat cut_end_3.txt |awk 'NR==2{print $2}')", >> $work_dir/seq_info_in_order_level.txt
     printf "$(cat Bits_mean_3.txt |awk 'NR==2{print $2}')", >> $work_dir/seq_info_in_order_level.txt
     printf "$(cat length_D3.txt |awk 'NR==2{print $2}')", >> $work_dir/seq_info_in_order_level.txt
   ...
   #4. get the short sequences  
     ...
     cat database_1.aln | seqkit seq -g -l |seqkit seq -m 80 > ${Order}_short_1.fasta
     cat database_2.aln | seqkit seq -g -l |seqkit seq -m 80 > ${Order}_short_2.fasta
     cat database_3.aln | seqkit seq -g -l |seqkit seq -m 80 > ${Order}_short_3.fasta
  ...
  seqkit stat ${Order}_short_3.fasta |awk 'NR==2' >> $work_dir/seq_stat_in_order_level.txt
